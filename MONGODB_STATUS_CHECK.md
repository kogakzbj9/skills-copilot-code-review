...
## ⚠️ 重要：MongoDBは自動起動が期待値です

**Codespacesでは、MongoDBは自動的に起動されるように設定されています。**

- Codespace作成時（初回）に自動インストール＆起動
- Codespace起動時（毎回）に自動起動

もしMongoDBが起動していない場合は、何らかの問題が発生しています。
詳細は [MONGODB_AUTOSTART.md](./MONGODB_AUTOSTART.md) を参照してください。
